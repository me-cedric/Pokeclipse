...
export default function InputBox({
  ...
  const usedSize = size > (minSize || 0) ? size : minSize || 0;

  useEffect(() => {
    if (arrayValue.length === 0) {
      console.log("init array");
      initArray(arrayValue, usedSize);
    }
  }, [arrayValue, usedSize]);

  useEffect(() => {
    if (arrayValue.length === 0 && value && value.length === usedSize) {
      console.log("init array with value");
      setArrayValue(convertValueToArray(usedSize, value));
    }
  }, [arrayValue.length, usedSize, value]);
